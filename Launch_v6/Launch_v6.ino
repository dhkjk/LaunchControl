//By: Dae Kim
//First arduino file to test on car without encoder
#include <LiquidCrystal.h> //include LicquidCrystal Library

//variable for state
int state = 0;//current state
int l_delay = 0;//The amount of launch control delay
int real_delay = 0;
 ...
 //delays were added to debounce
  ...
  while(digitalRead(CLUTCHPORT) == LOW && digitalRead(LAUNCHPORT) == LOW){
    //digitalWrite(REV_LIMITPORT, HIGH);
      ...
      if (digitalRead(CLUTCHPORT) == HIGH){
        state = state +1;
      }
  }
...
while (state == 2){ //Sustain mode
      ...
      delay(2);
      //l_delay
  delay(l_delay * 1000); //this is where the l_delay is going to go (a number is just added to make sure it was in Sustain mode)
  state = state +1;
